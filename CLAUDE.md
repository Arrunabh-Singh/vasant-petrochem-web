...
- **Tally Prime sync**: Tally runs in the Parallels VM (private IP, unreachable from Vercel) so the sync runs on the **Mac Hub**, not Vercel. `11_IT_Ops/Scripts/tally_sync.py` pulls Companies / Ledgers / Stock Items over HTTP:9000 (per-company collection export), captures raw XML + parsed JSON to `02_Accounts/Tally_Data/sync/`, and upserts into Supabase `tally.*` (migration `20260828140000_tally_sync.sql`) via the service-role REST API. `tally_sync.sh` wraps it and files an AI review card; it's wired into `run_all.sh`. Service-role key lives only in `11_IT_Ops/.supabase_service_role_DO_NOT_SHARE.txt` (excluded from rclone/Syncthing). Web view: `/admin/accounts/tally` (nav "Tally") lists companies + ledgers + stock with last-sync status. Stock *quantity* mapping is best-effort — verify on first live run via the JSON snapshots.
